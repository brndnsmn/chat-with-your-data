    ...
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-2">
        ...
        <button
            ...
            onClick={async () => {
              // CSV Export
              const { toCSV, downloadBlob } = await import("@/lib/utils");
              const csv = toCSV(Array.isArray(data) ? data : []);
              downloadBlob(`${title || "chart"}.csv`, new Blob([csv], { type: "text/csv;charset=utf-8" }));
            }}
          >Export CSV</button>
          <button
            className="text-xs px-2 py-1 border rounded"
            onClick={async () => {
              // Copy Link
              const payload = { chartType, title, description, data };
              const encoded = encodeURIComponent(
                btoa(unescape(encodeURIComponent(JSON.stringify(payload))))
              );
              const shareUrl = `${location.origin}${location.pathname}?chart=${encoded}`;
